...
get_data() {
  # todo: make the host key available
    scp -vvv -o StrictHostKeyChecking=no -i /secrets/keys/id_rsa -P $STORAGE_PORT -r mercure@$STORAGE_IP:/data/processing/$NOMAD_META_PATH/in/ ${NOMAD_ALLOC_DIR}/data/in
    ...
    chown mercure "${NOMAD_ALLOC_DIR}"
    chown mercure "${NOMAD_ALLOC_DIR}/data"
    chown mercure "${NOMAD_ALLOC_DIR}/data/out"
    echo "Data retrieved."
}
...
put_data() {
  ...
  scp -o StrictHostKeyChecking=no -i /secrets/keys/id_rsa -P $STORAGE_PORT -r ${NOMAD_ALLOC_DIR}/data/out mercure@$STORAGE_IP:/data/processing/$NOMAD_META_PATH
