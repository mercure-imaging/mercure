...
import pytest
from common import config, notification
...
p = get_params(action=["process", "both"], on_reception=TF, on_completion=TF,
               on_request=TF,
               do_request=TF,
               do_error=TF,
               on_error=TF)
p_route = get_params(action=["route"],
                     on_reception=TF, on_completion=TF,
                     on_request=False, do_request=False, do_error=TF,
                     on_error=TF)
p_notification = get_params(action=["notification"],
                            on_reception=TF, on_completion=TF,
                            on_request=False, do_request=False,
                            do_error=False, on_error=False)
...
async def test_notifications(fs, mercure_config: Callable[[Dict], Config], mocked: MockerFixture,
    ...
    if action == "notification":
        ...
        notification.trigger_notification_for_rule.assert_called_with(  # type: ignore
            "catchall", task_id, mercure_events.RECEIVED, tags_list=unittest.mock.ANY)
    ...
    if not do_error:
        ...
        assert notification.trigger_notification_for_rule \
            .spy_return == on_completion or (on_request and do_request)  # type: ignore
