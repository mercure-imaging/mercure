...
def test_backup_input_images(fs):
...
@pytest.mark.asyncio
async def test_restart_processing_job_success(fs, monkeypatch):
    ...
    fs.create_file(as_received_folder / mercure_names.TASKFILE, contents='{"id": "test-task", "process": {"module_name": "test_module", "status": "completed"}}')
