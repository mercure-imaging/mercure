...
def test_is_read_for_sending(fs):
    ...
    target = {"info": dummy_info, "dispatch": {"target_ip": "0.0.0.0", "target_port": 104, "target_aet_target": "ANY"}}
...
def test_read_target(fs):
    ...
    assert read_target
    assert "target_ip" in read_target.dict()
    assert "target_port" in read_target.dict()
    assert "target_aet_target" in read_target.dict()


def test_read_target_with_missing_key(fs):
    target = {"info": dummy_info, "dispatch": {"target_port": 104, "target_aet_target": "ANY"}}
