...
def test_is_not_offpeak():
    is_not = helper._is_offpeak("22:00", "06:00", _to_time("21:00"))
    assert not is_not


def test_is_offpeak():
    is_ = helper._is_offpeak("22:00", "06:00", _to_time("23:00"))
    assert is_


def test_other_input_offpeak():
    is_ = helper._is_offpeak("22:00", "6:00", _to_time("5:00"))
    assert is_


def test_wrong_start_input_offpeak():
    is_ = helper._is_offpeak("asdf", "6:00", _to_time("5:00"))
    assert is_


def test_wrong_end_input_offpeak():
    is_ = helper._is_offpeak("22:00", "asdf", _to_time("5:00"))
    assert is_
...
def test_emergency_cleaning_same_disk(fs: FakeFilesystem, mercure_config):
    ...
    emergency_clearing_level: float = config.emergency_clean_percentage / 100.0
